            ...
            {widgets.map(widget => {
                ...
                try {
                    ...
                    if (widget.type === 'recent_posts') {
                        const isArchiveContext = page.category === '补档' || page.is_old === true;
                        if (isArchiveContext) {
                            currentSite = Object.assign({}, site, {
                                posts: site.posts.filter(post => post.is_old === true)
                            });
                        }
                    }

                    return <Widget site={currentSite} helper={helper} config={config} page={page} widget={widget} />;
                } catch (e) {
